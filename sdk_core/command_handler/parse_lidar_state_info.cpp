...
bool
ParseLidarStateInfo::Parse(const CommPacket & packet, std::string & info_str)
{
...
bool
ParseLidarStateInfo::ParseStateInfo(const CommPacket & packet, DirectLidarStateInfo & info,
                                    std::set<ParamKeyName> & key_mask)
{
    uint16_t offset = 0;
    uint16_t key_num = 0;
    ...
    // printf("Lidar state info, pcl_data_type:%d, pattern_mode:%d, lidar_ip:%s, lidar_submask:%s,
    // lidar_gatway:%s.\n",
    //     info.pcl_data_type, info.pattern_mode, info.lidar_ip_info.ip_addr,
    //     info.lidar_ip_info.net_mask, info.lidar_ip_info.gw_addr);

    // printf("Lidar state info, host_ip_addr:%s, host_state_info_port:%u,
    // lidar_state_info_port:%u.\n",
    //     info.host_state_info.host_ip_addr, info.host_state_info.host_state_info_port,
    //     info.host_state_info.lidar_state_info_port);

    // printf("Lidar state info, host_ip_addr:%s, host_point_data_port:%u,
    // lidar_point_data_port:%u.\n",
    ...
    //        info.install_attitude.yaw_deg, info.install_attitude.x, info.install_attitude.y,
    //        info.install_attitude.z);

    // printf("Lidar state info, fov cfg0, yaw_start:%d, yaw_stop:%d, pitch_start:%d,
    // pitch_stop:%d.\n",
    //     info.fov_cfg0.yaw_start, info.fov_cfg0.yaw_stop, info.fov_cfg0.pitch_start,
    //     info.fov_cfg0.pitch_stop);

    // printf("Lidar state info, fov cfg1, yaw_start:%d, yaw_stop:%d, pitch_start:%d,
    // pitch_stop:%d.\n",
    //     info.fov_cfg1.yaw_start, info.fov_cfg1.yaw_stop, info.fov_cfg1.pitch_start,
    //     info.fov_cfg1.pitch_stop);

    // printf("Lidar state info, fov_en:%u, work_mode:%u, imu_data_en:%u, sn:%s,
    // product_info:%s.\n",
    //     info.fov_en, info.work_mode, info.imu_data_en, info.sn, info.product_info);

    // std::string version_app = std::to_string(info.version_app[0]) + ":" +
    // std::to_string(info.version_app[1]) + ":" +
    //     std::to_string(info.version_app[2]) + ":" + std::to_string(info.version_app[3]);

    // std::string version_load = std::to_string(info.version_load[0]) + ":" +
    // std::to_string(info.version_load[1]) + ":" +
    ...
    //     std::to_string(info.version_hardware[2]) + ":" +
    //     std::to_string(info.version_hardware[3]);
    ...
    // printf("Lidar state info, cur_work_state:%u, core_temp:%d, powerup_cnt:%u,
    // local_time_now:%lu, last_sync_time:%lu, time_offset:%ld.\n",
    //     info.cur_work_state, info.core_temp, info.powerup_cnt, info.local_time_now,
    //     info.last_sync_time, info.time_offset);
...
void
ParseLidarStateInfo::ParseLidarIpAddr(const CommPacket & packet, uint16_t off,
                                      DirectLidarStateInfo & info)
{
    uint8_t lidar_ip[4];
...
void
ParseLidarStateInfo::ParseStateInfoHostIPCfg(const CommPacket & packet, uint16_t off,
                                             DirectLidarStateInfo & info)
{
...
void
ParseLidarStateInfo::ParsePointCloudHostIpCfg(const CommPacket & packet, uint16_t off,
                                              DirectLidarStateInfo & info)
{
    ...
    std::string host_point_cloud_ip_str = std::to_string(host_point_cloud_ip[0]) + "."
                                          + std::to_string(host_point_cloud_ip[1]) + "."
                                          + std::to_string(host_point_cloud_ip[2]) + "."
...
void
ParseLidarStateInfo::ParseImuDataHostIpCfg(const CommPacket & packet, uint16_t off,
                                           DirectLidarStateInfo & info)
{
...
void
ParseLidarStateInfo::ParseIpCfg(const CommPacket & packet, uint16_t off, LivoxIpCfg & cfg)
{
    std::string ip_str
        = std::to_string(packet.data[off]) + "." + std::to_string(packet.data[off + 1]) + "."
...
void
ParseLidarStateInfo::LivoxLidarStateInfoToJson(const DirectLidarStateInfo & info,
